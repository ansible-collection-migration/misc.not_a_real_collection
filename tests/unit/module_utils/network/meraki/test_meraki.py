...
from ansible_collections.misc.not_a_real_collection.tests.unit.compat import unittest, mock
from ansible.module_utils.basic import AnsibleModule
from ansible_collections.misc.not_a_real_collection.plugins.module_utils.network.meraki.meraki import MerakiModule, meraki_argument_spec, HTTPError, RateLimitException
...
def test_fetch_url_404(module, mocker):
    url = '404'
    mocker.patch('ansible_collections.misc.not_a_real_collection.plugins.module_utils.network.meraki.meraki.fetch_url', side_effect=mocked_fetch_url)
    mocker.patch('ansible_collections.misc.not_a_real_collection.plugins.module_utils.network.meraki.meraki.MerakiModule.fail_json', side_effect=mocked_fail_json)
...
def test_fetch_url_429_success(module, mocker):
    url = '429'
    mocker.patch('ansible_collections.misc.not_a_real_collection.plugins.module_utils.network.meraki.meraki.fetch_url', side_effect=mocked_fetch_url_rate_success)
    mocker.patch('ansible_collections.misc.not_a_real_collection.plugins.module_utils.network.meraki.meraki.MerakiModule.fail_json', side_effect=mocked_fail_json)
    mocker.patch('time.sleep', return_value=None)
